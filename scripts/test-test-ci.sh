...
readonly SCRIPT_DIR
readonly TARGET_SCRIPT="${SCRIPT_DIR}/test-ci.sh"
...
test_workflow_file_exists() {
    local workflow_file="${SCRIPT_DIR}/.github/workflows/ci-pipeline.yml"
    [[ -f "$workflow_file" ]]
    assert_success "CI workflow file exists at .github/workflows/ci-pipeline.yml"
}

test_script_references_correct_workflow() {
    assert_file_contains "$TARGET_SCRIPT" ".github/workflows/ci-pipeline.yml" \
        "Script references correct workflow file"
}

test_script_uses_pull_request_event() {
    assert_file_contains "$TARGET_SCRIPT" "act pull_request" \
        "Script uses pull_request event"
}

test_script_creates_event_payload() {
    assert_file_contains "$TARGET_SCRIPT" "pr-event.json" \
        "Script creates event payload file"
}
...
test_script_gets_current_branch() {
    assert_file_contains "$TARGET_SCRIPT" "git rev-parse --abbrev-ref HEAD" \
        "Script retrieves current branch name"
}
...
test_script_sets_artifact_path() {
    assert_file_contains "$TARGET_SCRIPT" "artifacts-ci" \
...
test_event_payload_structure() {
    # Test that the script can generate a valid event payload
    local test_payload="${TEST_DIR}/test-payload.json"
...
EOF
    ...
    if command -v jq >/dev/null 2>&1; then
        jq empty "$test_payload" 2>/dev/null
        assert_success "Generated event payload is valid JSON"

        # Verify structure
        local head_ref
        head_ref=$(jq -r '.pull_request.head.ref' "$test_payload")
        assert_equals "$test_branch" "$head_ref" "Event payload contains correct head ref"

        local base_ref
        base_ref=$(jq -r '.pull_request.base.ref' "$test_payload")
        assert_equals "master" "$base_ref" "Event payload contains correct base ref"
    else
        TESTS_RUN=$((TESTS_RUN + 1))
        echo -e "${YELLOW}!${NC} Skipping JSON validation tests (jq not available)"
        TESTS_PASSED=$((TESTS_PASSED + 1))
    fi
}
...
print_summary() {
    echo ""
    echo "======================================"
    echo "Test Summary for test-ci.sh"
    echo "======================================"
...
main() {
    ...
    echo "Running tests for test-ci.sh..."
    echo ""
